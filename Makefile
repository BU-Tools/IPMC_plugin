...
LIBRARY_IPMISOL_DEVICE_SOURCES = $(wildcard src/IPMISOL_device/*.cc)
LIBRARY_IPMISOL_DEVICE_OBJECT_FILES = $(patsubst src/%.cc,obj/%.o,${LIBRARY_IPMISOL_DEVICE_SOURCES})

LIBRARY_IPMISOL_CLASS = lib/libBUTool_IPMISOLClass.so
LIBRARY_IPMISOL_CLASS_SOURCES = $(wildcard src/IPMISOL_Class/*.cc)
LIBRARY_IPMISOL_CLASS_OBJECT_FILES = $(patsubst src/%.cc,obj/%.o,${LIBRARY_IPMISOL_CLASS_SOURCES})
...
build: _all
buildall: _all
_all: _cactus_env ${LIBRARY_IPMISOL_DEVICE} ${LIBRARY_IPMISOL_CLASS}
...
# ------------------------
${LIBRARY_IPMISOL_DEVICE}: ${LIBRARY_IPMISOL_DEVICE_OBJECT_FILES} ${IPBUS_REG_HELPER_PATH}/lib/libBUTool_IPBusIO.so ${LIBRARY_IPMISOL_CLASS}
	g++ ${LINK_LIBRARY_FLAGS} -lBUTool_IPMISOLClass ${LIBRARY_IPMISOL_DEVICE_OBJECT_FILES} -o $@
	@echo "export BUTOOL_AUTOLOAD_LIBRARY_LIST=\$$BUTOOL_AUTOLOAD_LIBRARY_LIST:$$PWD/${LIBRARY_IPMISOL_DEVICE}" > env.sh

${LIBRARY_IPMISOL_CLASS}: ${LIBRARY_IPMISOL_CLASS_OBJECT_FILES} ${IPBUS_REG_HELPER_PATH}/lib/libBUTool_IPBusIO.so
	g++ ${LINK_LIBRARY_FLAGS}  ${LIBRARY_IPMISOL_CLASS_OBJECT_FILES} -lipmiconsole -o $@
